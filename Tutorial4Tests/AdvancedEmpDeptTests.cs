...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, e => e.DeptNo, d => d.DeptNo, (e, d) => new { Emp = e, Dept = d })
            .Join(grades, ed => ed.Emp.Sal, g => g.Losal, (ed, g) => new { ed.Emp.EName, ed.Dept.DName, Grade = g.Grade, EmpSal = ed.Emp.Sal, Losal = g.Losal, Hisal = g.Hisal })
            .Where(edg => edg.EmpSal >= edg.Losal && edg.EmpSal <= edg.Hisal)
            .Select(edg => new { edg.EName, edg.DName, edg.Grade })
            .ToList();
