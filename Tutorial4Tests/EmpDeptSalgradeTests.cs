...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        var dept30 = depts.Where(d => d.Loc == "CHICAGO").Select(d => d.DeptNo).ToList();
        
        List<Emp> result = emps.Where(e => dept30.Contains(e.DeptNo)).ToList();
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.Join(grades, e => 1, s => 1, (e, s) => new { e.EName, e.Sal, Grade = s.Grade, Losal = s.Losal, Hisal = s.Hisal })
            .Where(eg => eg.Sal >= eg.Losal && eg.Sal <= eg.Hisal)
            .Select(eg => new { eg.EName, eg.Grade })
            .ToList();
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.Where(e => e.Sal > avgSalaries.FirstOrDefault(a => a.DeptNo == e.DeptNo).AvgSal)
            .Select(e => e.EName)
            .ToList();
